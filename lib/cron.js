(function(exports) {
  ...
  var updateServerResult = function(result, addIfNotExists, overrideStatus, addIconOnly) {
    ...
    if(f) {
      if(f.ignore) {
        if(addIfNotExists)
          ...
          if(addIconOnly && overrideStatus && !!result.status && !!result.status.icon) {
            if(typeof f.status === "object") f.status.icon = result.status.icon;
            else f.status = { icon: result.status.icon };
          }
          return;
        }
      }
    ...
    } else if(addIconOnly && overrideStatus && !!result && !!result.status.icon) {
      serverstatus.push({
        hash: result.hash,
        status: {
  ...
  var cronFetchServerList = function(srcList, timeout) {
    return setTimeout(function() {
      return fetchServerList(srcList, function() {
        return cronFetchServerList(srcList, timeout);
      });
    }, timeout);
  };
  ...
  exports.run = function(broadcastUpdate, config) {
    if(!config) config = {};
    ...
    jsonfile.readFile("./cache.json", function(err, result) {
      ...
      fetchServerList(config.sourceList, function() {
        cronFetchServerList(config.sourceList, config.fetchServerListPeriod);
